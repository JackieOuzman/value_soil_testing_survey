...
```{r Q14 data graphs freq P, echo=FALSE}
##Table for frquency
...
```{r graph and table test _Q16 K a}
# table
 ##Table for frquency
...
# summary_stats_all_resp <- function(df, group_by, clm_in_df){  
#   df <- dplyr::filter( df, !! clm_in_df != "NA") %>% 
#     group_by(!! group_by) %>% 
#     count(!! clm_in_df) %>% 
#     mutate(
#       Resp_numb = case_when(Resp_Group == "Agronomist" ~ 71,
