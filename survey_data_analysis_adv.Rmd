...
* We have more grower(99) respondants than agronomist (71)
* For the growers by region we have some very low responses rates for certain regions (e.g. lower EP).
  This regional grouping (for growers) cannot be used as is, we will need to be merged results are to reflect regional differences.
...
* Can't combine the data for this - perhaps indicates a pattern for the whole study (combining the two groups, growers with argosomist should be done with causion).
...
ggplot(Q3_regions_agro_grower_narrow, aes(x = crop, y = ave_area))+
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1),
        plot.caption = element_text(size = 8)) +
  scale_x_discrete("crop", labels=c("Canola", "Ceral", "Fallow", "Hay", "Pasture", "Pulse"))+
  labs(x= "Regions",
       y = "Croped area",
       title = "Avearge area by crop",
       subtitle = "Agronomist and grower",
       caption = "Q)  Which of the following crops do you grow - percentage by area? NB area of cropped land is over the last 3 seasons")
...
We have cal the area and percenatge of crop.
...
ggplot(Q4_resp_percent, aes(Q_4, percent))+
  ...
  facet_wrap(~Resp_Group)+
  theme_classic()+
  labs(x= "percentage of total varaible cost",
       y = "percent response",
       title = "Percent of annual variable costs attriubuted to fertilisers",
       #subtitle = "Agronomist and grower",
...
I have another bin called 'other' which is used to get the percenatge.
This just mean that I am calulating the percenatge based on all the respondants (not the NA)
