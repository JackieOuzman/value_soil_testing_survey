...
test <- rbind(Q_5.2_N, Q_5.2_P, Q_5.2_S,  Q_5.2_Zn,
              Q_5.2_Cu, Q_5.2_Mn, Q_5.2_pH, Q_5.2_Fe, Q_5.2_Mg ) 


  ggplot( test, aes(nutrient, percent))+
  geom_col()+
  facet_wrap(. ~ Resp_Group)+
  theme_classic()+
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L))
...
#### Up to here need to arrange N, P 
...
ggplot(Q6_all_agro, aes(Q_6, percent))+
  geom_col()+
  geom_text(aes(label = percent(signif(percent),0), vjust = -1.0, hjust = 0.5))+
  theme_classic()+
  facet_wrap(~ Resp_Group)+
  labs(x= "All regions",
       y = "Recommend testing",
...
#head(Q6_by_Agro_Region)
#head(Q6_by_grower_Region)
Q6_by_Agro_Region <- mutate(Q6_by_Agro_Region, Resp_Group = "Agronomist")
Q6_by_grower_Region <- mutate(Q6_by_grower_Region, Resp_Group = "Grower")

Q6_by_Agro_grower_Region <- rbind(Q6_by_Agro_Region, Q6_by_grower_Region)

filter(Q6_by_Agro_grower_Region, Q_6 == "No") %>% 
ggplot(aes(names_display, percent))+
  geom_col()+
  facet_wrap(~ Resp_Group)+
  geom_text(aes(label = percent(signif(percent),0), vjust = -1.0, hjust = 0.5))+
...
```{r Q6 agro summary grower saying no, eval=FALSE, include=FALSE}
